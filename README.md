...
aachat session run design-agent --project <project> "<案件ブリーフ>"
```
...
- `.agents/skills/`: この agent 専用の実行時 skill。フェーズ軸で 6 つ（requirements-intake / first-view-proposal / design-system / layout-design / nextjs-implement / design-review）
...
`description_ja` / `description_en` は Discovery submit 時の説明文です。repo 内の固定ファイルではありません。
